...
async function runAuthentication() {
  console.log('====================================================');
  console.log('          BatiFlow Instagram Authenticator          ');
  console.log('====================================================');
  console.log('This tool will open a headful Google Chrome instance.');
  console.log('Please log into Instagram (instagram.com) in the browser window.');
  console.log('Once you are fully logged in and see your home feed,');
  ...
  // 2. Open tab and load Instagram login page
  const tabPool = new TabPool();
  const client = await tabPool.acquireTab();
  
  console.log('[Auth] Loading Instagram...');
  await client.navigate('https://www.instagram.com/accounts/login/');
  ...
  try {
    ...
    console.log(`[Auth] Captured ${cookies.length} Instagram session cookies.`);
    
    // 5. Encrypt and save cookies
    const profile = new ProfileManager();
    profile.encryptAndSaveSession(cookies, 'instagram');
    
    console.log('\n✅ Success! Instagram session cookies are encrypted and securely stored.');
  } catch (err) {
