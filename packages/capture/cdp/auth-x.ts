...
async function runAuthentication() {
  console.log('====================================================');
  console.log('             BatiFlow X Authenticator               ');
  console.log('====================================================');
  console.log('This tool will open a headful Google Chrome instance.');
  console.log('Please log into X (Twitter) in the browser window.');
  console.log('Once you are fully logged in and see your timeline,');
  ...
  // 2. Open tab and load X login page
  const tabPool = new TabPool();
  const client = await tabPool.acquireTab();
  
  console.log('[Auth] Loading X (Twitter)...');
  await client.navigate('https://x.com/login');
  ...
  try {
    ...
    console.log(`[Auth] Captured ${cookies.length} X session cookies.`);
    
    // 5. Encrypt and save cookies
    const profile = new ProfileManager();
    profile.encryptAndSaveSession(cookies, 'x');
    
    console.log('\n✅ Success! X (Twitter) session cookies are encrypted and securely stored.');
  } catch (err) {
